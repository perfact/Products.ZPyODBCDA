...
def addpyodbcConnectionBrowser(self, id, title, connection_string,
                               auto_commit=None, connected=None, MaxRows=10,
                               REQUEST=None):
...
class pyodbcConnectionBrowser(pyodbcConnection):
    ...
    def __is_foreign_key(self, description):
        res = 1
        if description.find('FOREIGN') == -1:
            res = 0
        return res

    def __get_source_table(self, arguments):
        res = ''
        res = arguments.split('\\000')[2]

        return res

    def __get_source_field(self, arguments):
        res = arguments.split('\\000')[4]
        return res
